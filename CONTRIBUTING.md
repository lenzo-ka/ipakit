...
- **Say which orderings are load-bearing**, and expect that claim to be measured as an exact one. List them in `LOAD_BEARING` in `tests/test_rule_sets.py`; the sweep there permutes every pair of rules in your set and asserts that the permutations which move an answer are exactly the ones putting a listed ordering the wrong way round, and then lifts each rule you did not list to every position in the cascade and asserts it survives all of them. A dependency you left out fails with the pair of rules named, and an ordering you claimed that the corpus cannot reach fails with the block named.

...
- `articulator` declared `axis="+x"`, ordered by home position along the tract, while the metric scored it as an identity and `docs/distance.md` listed it among the categorical dimensions and omitted it from the axes table. It declares no axis. The organ that moves is not a position, and a position for it would be a second declaration.

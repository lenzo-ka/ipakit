...
- CLTS vowel and consonant duration declarations map to distinct house length values when the source asserts them.
- The CLTS gap report records enhancement candidates, their evidence and dispositions, including the additions accepted for lane H.
...
- The MFA phone set ships as one declaration per MFA-phone-set dictionary plus their union, generated by `scripts/mfa_vocabularies.py` from a pinned `mfa-models` revision and gated by `make mfa-vocabularies-check` and CI. A dictionary phone is one phone by declaration, so a phone MFA writes as a sequence takes the house tie (`aj` is `a͜j`, `tʃ` is `t͡ʃ`, and `kp` is `k͡p`) and every atom is exactly one house unit. `MFABridge` takes a declaration name, while `MFA` stays English. Phones carrying a mark ipakit does not declare are refused with the codepoint named rather than spelled approximately.
- Named phoneset inventories and notation styles, read by `ipakit.phoneset_mapping`, `ipakit distance map`, and the new `ipakit inventory`. [docs/inventories.md](docs/inventories.md).
...
- The mid-sagittal reference figure draws the rest tongue. It is the landmark key, and it named every place while omitting the articulator that reaches them, so the labels sat in empty space and the figure could not be read against a posed frame. The surface comes from the head's declared home controls, the same ones a phone is drawn from. Figures for individual phones are unchanged.
- A negative tongue offset is refused rather than clamped to the floor. Clamping above stays: an offset past 1.0 is a constriction pressed beyond contact, and the roof is where it belongs. Below is a different case — a tongue behind the floor is not a posture, and drawing it at zero would render a plausible picture from a broken declaration. Unreachable from the shipped inventory, whose minimum constriction offset is 0.05 and which declares no negative tongue control, so this fires only for a declaration that has gone wrong.
- `protrusion` bridge: lip rounding is one gesture whether a vowel spells it `rounded` or a consonant spells it `labialized`. Only the vowel spelling reached the metric before, because a secondary articulation is carried by the weighted place components and a labial base already sits at the place its own rounding would move it toward, so `d(p, pʷ)` was 0. Every distance moves; `confusion.json` and the alignment baseline are regenerated, and the shipped fingerprint changes.
...
- `unmodeled` asked whether a feature declares tract coordinates, not whether this posture read one, so a bundle stating a consonantal manner over a vowel drew half of it and reported nothing missing; `tract_reading` answers what the point was read from, and the annotation layer asks that.

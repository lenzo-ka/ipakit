...
class TestRepresentationCommands:
    def test_to_json_emits_the_complete_representation(self, monkeypatch, capsys):
        ...
        assert representation["format_version"] == "0.2.0"
...
class TestInputThatWasNotReadInFullReachesTheExitStatus:
    ...
    def test_the_converters_no_longer_drop_in_silence(self, monkeypatch, capsys):
        ...
        assert rc == LOSSY
        # A converter owns the whole call's diagnostics even when the IPA
        # reader is the layer that first recognizes the loss.
        assert "unconvertible symbol(s) ['@'] converting to CMU ARPABET" in err
        assert "while parsing IPA" not in err

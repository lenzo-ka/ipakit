...
def _opening(name: str, artifact: str) -> str:
    ...
    return (
        ...
        f"artifact={quoteattr(artifact)} license={quoteattr(LICENSE)} "
        f"kind={quoteattr(KIND)} tier={quoteattr('espeak-' + name)} "
        'source-style="text">'
    )
...
def render(name: str, inventory: OrderedDict[str, Phone]) -> tuple[bytes, Counter[str]]:
    ...
    lines = [
        ...
        _opening(name, "1.52.0 phsource phoneme tables"),
        "  <round-trip>",
...
def english_compatibility_bytes(inventory: OrderedDict[str, Phone]) -> bytes:
    ...
    index = next(i for i, line in enumerate(lines) if line.startswith("<vocabulary "))
    lines[index] = _opening("en", "1.52.0 phsource/phonemes and phsource/ph_english")
    return ("\n".join(lines) + "\n").encode()

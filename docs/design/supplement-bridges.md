...
| Do projections earn admission? | **No.** One must be total over its fine feature, so a supplement declaring one redefines what the base said rather than extending it. |
...
**Refuse, and not for symmetry.** Two reasons, in order of weight.
...
**It is not read by the write side only.** The comment beside the block said so, and `compose_unit` is indeed the main reader, but `ipakit/tract.py` reads it too: `glottal_aperture` puts a bundle spelling only `voiced` at the center of the phonations that read that way, which is how a coarse spelling reaches the drawing at all. The comment says read-rather-than-resolved, which is the part that was load-bearing — nothing writes a projection back onto a segment's features — and it names the readers.

What a second projection does to a drawing is worth measuring rather than assuming, so it was, over `length`, the same way §2's bridges were:
...
Neither reason is fatal on its own. Together they say a projection is a statement about the whole feature space made by a file that is not allowed to describe the feature space, and that a reader of that file cannot see where its consequences land.
...
### (e) `glottal_aperture` reads the axis, and this is done

Turned up by §5 and unrelated to supplements: the scale used to be found by discovery — the first ordinal feature a projection refined, in alphabetical order — so a second projection would have taken the folds over with nothing raised and every figure redrawn.

The scale is the feature that declares `axis="+glottal-aperture"`, which `phonation` does. Two features declaring it is refused with both names, because that is the choice no code should make; an inventory declaring it nowhere draws the folds without a state, as one declaring no median aperture draws no folds. `tests/test_tract_figures.py::TestTheGlottalScaleIsDeclaredAndNotDiscovered` perturbs the declaration each of those three ways.

...
**Could not be reached at all.** Kenstowicz & Kisseberth, *Generative Phonology* (1979), and Kenstowicz, *Phonology in Generative Grammar* (1994) — every archive.org OCR file returns 401 and search-inside reports no text without a loan, exactly as [morph-boundary.md](morph-boundary.md) records. **No secondary source reporting either book's treatment of braces was found either**, so nothing is attributed to them here, at any remove. McCawley's "On the role of notation in generative phonology" is the classic critique of the abbreviatory apparatus and is paywalled; it is named here and not relied on.
...
ipakit has no evaluation measure and is not going to grow one. §11 is what it can have instead, and it is less than *SPE* had.

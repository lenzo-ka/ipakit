...
- **`features("aː")["length"]` is `"normal"`.** The scalar read warns that it omitted `ː` and points to `feature_values("aː")`, which reports `length=("long",)`. Ignoring that warning can still produce the spurious disagreements measured in §2.

...
The constriction-location row is where the gap is widest, and the reason is in the declarations rather than in the anatomy. A vowel may state where it constricts — `constriction-location`, one of the places `place` locates — and nineteen do, in the four families Wood (1979) reproduces over 40 subjects in 13 languages and names again, at greater length, in Wood (1990) and in his own summary of the 1979 figure. Every other vowel states none and takes the older reading, `arc` from `backness`:
...
A phone's target is a *constriction* specification (articulator, location, degree). Solving for the posture that achieves it is inverse kinematics, and is underdetermined — several postures reach the same constriction. That indeterminacy is real, not a modeling defect: it is what makes motor equivalence possible, and a renderer resolves it with a cost function (least movement from the previous posture, typically), which is also what produces plausible coarticulation.
...
[tract-figures.md](tract-figures.md) draws the declared geometry — a labeled reference at rest, and a figure per phone chosen to show one contrast each — regenerated by `make figures` and checked in. [tract-reference.md](tract-reference.md) carries the declared positions and the provenance of each number. The drawing goes through `Head.project` and computes nothing of its own, which is what made it useful as a check: a projection defect showed up there as a wall crossing itself while the numbers looked ordinary.

The figures also carry an **annotation layer** for what a posture of two parameters cannot express — glottal state, a secondary constriction, a release phase, an out-of-plane channel. Which of those is drawn as geometry and which is annotated is decided by the feature's own declaration, and the layer reads the tract; it never defines it. `glottal_aperture` is the one that comes closest to geometry: §4.5 lists glottal aperture as one of the nine posture parameters, and the folds are drawn from it, but it is read off whichever feature declares the glottal-aperture axis rather than solved for, and `tract_point` is untouched.
...
- **Where lateral information lives.** Mid-sagittal geometry cannot represent grooving. An annotation on the segment is the pragmatic answer and is what the figures now do — `channel` declares `axis="+z"`, and any feature on that axis is annotated with a mark that says *out of plane* rather than given a contour it cannot have. A second (coronal) plane is still the faithful answer, and it would turn that mark into a picture.
- **Airstream has a direction and the data does not say so.** Direction is the whole content of the ejective/implosive/velaric contrast, and an arrow is the obvious schematic for it, but no value `airstream` declares carries a direction. Drawing the arrow means a renderer-side table of which way each value points, which is exactly the kind of restated fact this repo has removed three times. What it wants is an attribute on the value — the way a `place` value carries its `arc` — after which the figure derives the arrow and the metric gains an ordering it does not have today. Until then the figures name the airstream and draw no arrow. The velaric case is the exception that shows the shape of the fix: `airstream="velaric"` is enough for `constrictions` to derive a click's velar closure, and that closure *is* drawn.
...
- **The port is not declared as a junction, so this geometry cannot be simulated as it stands.** Every head lands the nasal branch's last point on the oral wall at the port arc, so the two tubes share a boundary point — but a shared point is not a coupling, and nothing declares which apertures meet there or at what arc the three are to be sampled together. The velum's raised position reaches the branch's *midline* rather than a surface that closes against the roof, so the seal is a number rather than a boundary, whatever the velum is doing and including for an oral segment whose port is nominally sealed. Drawn, none of this shows: the filled shapes overlap. Given to a wave solver it is a coupling no parameter names. Closing the outline in the *drawing* is cosmetic. It matters only if the sagittal outline is taken to be the acoustic model, and it should not be: a waveguide synthesizer does not need a watertight outline, it needs area functions and a junction. Pink Trombone (Neil Thapen, 2017, MIT) is the worked example — 44 oral sections and 28 nasal ones as 1D area arrays, meeting at a three-port scattering junction where exactly three areas sum, `A[noseStart] + A[noseStart+1] + noseA[0]`, with the velum not a flap but a single number, `noseDiameter[0]`, moving between 0.01 closed and 0.4 open.

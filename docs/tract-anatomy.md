...
In the declared geometry, the lowered oral face continues the hard-palate
boundary to the velar target. The tongue dorsum and the flap therefore meet at
one shared boundary on a velar closure. Raising moves the free edge from that
target to the posterior wall; the resulting edge-to-wall distance is the velic
aperture. Tongue height neither defines nor clips that gap.
...
The figures now draw a structured posture: oral and secondary constrictions, velic and glottal state, lip aperture and protrusion, and tongue controls. The **annotation layer** is only for information that this sagittal geometry cannot faithfully picture — a release phase or an out-of-plane channel, for example. Which distinctions become geometry and which remain annotations is decided by the feature's own declaration, and the layer reads the tract; it never defines it. The remaining flat-projection collisions are measured rather than described from memory:

```python
from ipakit import IPAFeatures, tract_svg

ipa = IPAFeatures()

def sagittal(phone):
    drawn = tract_svg.drawing("adult-male", phone, ipa)
    return tract_svg.section_svg(
        drawn["geometry"], None, drawn["aperture"], drawn["posture"],
        None, drawn["active"]
    )

...
**A relation, in the general sense.** A rule set is a *function from a form to a set*, not an arbitrary form-to-form relation. It cannot be inverted, intersected with another rule set, or complemented; there is no "the forms that both A and B derive" operator, because there is no operator that builds a rule set out of two rule sets except concatenation. "[T]he regular relations differ from the regular languages, however, in that they are not closed under intersection and complementation" (Kaplan & Kay 1994: 342), so an algebra offering the latter two would be claiming more than the finite-state account rather than catching up with it. Invertibility is classified rather than imposed as a new algebraic operator: `Rule.invertibility(phoneset)` first requires length preservation, then tests injectivity of the induced map on inventory phones that can occupy the environment. Clause 2 has two failures. An **escape** is an output-shaped phone that satisfies the environment but not A, so it stays unchanged while a moved phone maps onto it. An **absorption** is an output-shaped phone that satisfies both the environment and A but is a fixed point of the change, so it too stays unchanged while absorbing a moved phone's image. The polarity exchange `[voiced=α] -> [voiced=-α]` passes precisely because it has no fixed points, not because exchanges receive a special case. `RuleSet.invertibility(phoneset)` reports every rule, the first loss, and whether analysis uses the deterministic or capped-enumeration regime. This is inventory-relative: `n -> ŋ / _ k` passes when `ŋ` is not underlying and fails, naming `ŋ`, when it is. The report is also available as `ipakit rules invertibility`. Set intersection over two `VariantSet`s is of course available to a caller — the closure claims below are about which operations the *algebra* provides.

With the inventory `{d, t}`, the shipped German set's report is:

```text
german-final-devoicing: 1 rule(s)
  1  not invertible: clause 2 fails (absorption): underlying 't' is an absorption fixed point in this environment and collides with moved 'd'
set: invertibility is lost at rule 1, because clause 2 fails (absorption): underlying 't' is an absorption fixed point in this environment and collides with moved 'd'; regime: capped candidate enumeration
```

No shipped set is used as the deterministic-path corpus witness: that pin uses the polarity-exchange fixture above so its lack of fixed points is visible in the test itself.

...
| Raw distance and inventory-relative percentile positions | `Alignment`, `AlignmentStep`, `CostSchedule`, `Correspondence`, `Coverage`, `PhoneCost`, `PhonesetMapping`, `PhonesetComparison`, `PronunciationMatch`, `ScoringParameters`, `SequenceMatch`, `WordDistanceResult`, `DistanceModel`, `distance`, `segment_distance`, `pairwise_distances`, `word_distance`, `directional_word_distance`, `word_similarity`, `explain_word_distance`, `nearest_pronunciation`, `rank_pronunciations`, `sequence_distance`, `sequence_similarity`, `rank_sequences`, `normalized_distance`, `confusability`, `distance_model`, `phoneset_mapping`, `phoneset_comparison` | **CLI-reachable:** `distance pair`, `segment`, `matrix`, `confusability`, `word`, `nearest`, `seq`, `map`, and `compare`, including alignment/explanation output. `map` relates two phonesets in one selected way; `compare` reports the ordered sets, nearest mappings both ways, similarity matrix, source terms, directional means and worst cases, asymmetry, and caller-chosen `--coverage-at` thresholds. |
...
| Applicability-scoped matrices and inventory-relative percentile positions | `pairwise_distances`, `normalized_distance`, `confusability`, `distance_model`, `DistanceModel.derive` with `applicable_only=True` | **CLI-reachable:** `distance matrix --applicable-only` and `distance confusability --applicable-only`. |
| Applicability-scoped word distance | `word_distance`, `directional_word_distance`, `word_similarity`, `explain_word_distance`, `nearest_pronunciation`, `rank_pronunciations` with `applicable_only=True` | **CLI-reachable:** `distance word --applicable-only`, `distance directional --applicable-only`, and `distance nearest --applicable-only`, including raw and explanation output. |

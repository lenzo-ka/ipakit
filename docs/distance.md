...
`phoneset_mapping` accepts a named inventory or a `Phoneset`, iterable, or `Path` independently on either side: `phoneset_mapping("cmudict", "mfa")`, `phoneset_mapping(["p", "t"], "mfa")`, `phoneset_mapping("cmudict", Path("target.phones"))`, and `phoneset_mapping(Path("source.phones"), Path("target.phones"), source_style="wild")` each cover one combination of named and anonymous sides. Here `mfa` is the union of the shipped MFA declarations; a particular generated declaration is named `mfa:<name>`. A name supplies its phones and style, while an anonymous side defaults to house IPA and may select a named style; a style beside a named inventory is refused, and `wild` is accepted as a style but refused as a finite side. X-SAMPA and Kirshenbaum are explicit converters, not inventory styles.

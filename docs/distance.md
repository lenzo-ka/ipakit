...
| Word alignment | a gap costs `GAP_COST`, a substitution costs `(delete + insert) ×` the pair's dissimilarity, and `similarity` is normalized by the phone null alignment's cost; boundary claims do not enlarge that scale |
...
matching while suppressing only the distance claim the mark itself would
make; an explicit break at the same margin remains. Use
...
|---|---|---|
| `place`, `backness`, `constriction-location`, `articulator` | `+x` | lips → glottis |
| `height` | `+y` | jaw → palate |
| `tone` | `+f0` | low → high |
...
`explain_transcription_distance` exposes that arithmetic directly. Composite substitutions use flat qualified rows for matched parts, unmatched-part nearest comparisons and material, and junctures; they do not also include a parent `segmental` aggregate. A matched part remains one outer row even when its own atomic comparison can be explained one level down. Public `cost` values retain their full floating-point precision, so reconstruct the reported substitution distance exactly as `sum(cost * weight) / sum(weight)`, treating an omitted `weight` as `1.0`. When a composite and prosodic riders meet, the segmental rows explicitly share the fold's `max(part counts)` weight and every rider keeps weight 1; no reported cost is altered to force an unweighted mean. For `u͡i` / `u͜i`, all weights are 1 and the shipped rows are `matched part a[0]~b[0]: u/u = 0`, `matched part a[1]~b[1]: i/i = 0`, and `juncture a[0]~b[0]: fuse/seq = 1`, reconstructing `1/3`.
...
**The normalizer is the cost of the phone null alignment**, `n · delete + m · insert` — deleting every phone token of the first word and inserting every phone token of the second. Boundary claims spend on that scale but do not enlarge it: adding the null cost of the claims present would give each pair a different unit, violate the triangle inequality, and let a local window improve its denominator merely by carrying another matching claim. A fixed two-claim floor supplies the scale only for the degenerate case with no phones. `max(n, m)` is a different claim, and the difference is exactly on length mismatch: it charges a truncation once where this charges the material that went missing and the material that replaced it apart. Both transcription-distance paths — `IPAFeatures.transcription_distance` and `DistanceModel.transcription_distance` — read one function for this, so a caller who switches to the model to use inventory-relative weights changes which substitution costs the alignment sees and not what a similarity means.
...
**Word-level distance is a phone alignment plus asserted boundary claims.** Breaks remain relations rather than phone tokens, but an asserted break is no longer free against an unclaimed margin. U+203F UNDERTIE suppresses only its own distance claim while remaining a word edge for rules; it does not erase another explicit break at that margin. The shipped confusion matrix remains phone-only.
...
**The denominator sums over the phones.** `similarity` is `1 - edit_cost / denom`, where `denom` is the phone null alignment's cost: delete every phone of the first word, insert every phone of the second. Boundary claims add to `edit_cost` but not this denominator, so claim presence cannot change the scale. The phone budget is a sum over the actual phones, not a token count times a price. The two agree whenever the price is flat and disagree as soon as it is not.
...
together. Every candidate window uses the target's phone insertion budget as
the same denominator, so adding free context cannot lower a fit that remains
available. Pre-tokenized `sequence_distance` still has no boundary claims to
carry.
...
`phoneset_mapping` accepts an `Inventory`, a named inventory, or a `Phoneset`, iterable, or `Path` independently on either side: `phoneset_mapping("cmudict", "mfa")`, `phoneset_mapping(["p", "t"], "mfa")`, `phoneset_mapping("cmudict", Path("target.phones"))`, and `phoneset_mapping(Path("source.phones"), Path("target.phones"), source_style="wild")` each cover one combination of named and anonymous sides. Here `mfa` is the union of the shipped MFA declarations; a particular generated declaration is named `mfa:<name>`. An `Inventory` or name supplies its phones and style, while an anonymous side defaults to house IPA and may select a named style; a style beside an inventory is refused, and `wild` is accepted as a style but refused as a finite side. X-SAMPA and Kirshenbaum are explicit converters, not inventory styles.

On the command line: `distance map SOURCE TARGET` takes a registry name or a phoneset file on each side and prints each correspondence in both notations; collapses, unmapped phones, and unused targets use the same styles, with `-` where spelling is refused. `--from-style` and `--to-style` name the notation each file is written in, `--one-to-one` switches operations, `--max-distance` sets the refusal, `--wild` abbreviates wild style on both sides, `--no-tie` turns off the tied reading, and `-f json` gives the whole result. A token that is both a registry name and a file is refused; prefix the path with `./` to select the file. `inventory list` and `inventory show NAME` inspect the same registry. To transcode an inventory file exactly, `convert phoneset --from-style SOURCE --to-style TARGET` reads through house IPA and refuses the entire write if either leg cannot represent an entry; omit `--to-style` to write house IPA. That operation changes notation only. It never substitutes a nearest phone or claims target-inventory membership.

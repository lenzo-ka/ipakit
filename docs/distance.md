...
`arc` and `offset` are declared per feature value in `data/ipa.xml`: consonants take `arc` from `place` and `offset` from `manner`; vowels take `arc` from the `constriction-location` they state, or from `backness` where they state none, and `offset` from `height`. Sixteen vowels state one — the families Wood (1979) reproduces, read at the arcs `place` already declares — and the rest do not, so the rest take `backness`. That fallback is reported rather than passed off: `tract_reading` returns `backness` in `approximated` and `unmodelled` gives it kind `approximate`, because `backness` says where the tongue body is and not where it constricts. The consonant values are hand-placed from published mid-sagittal anatomy and are schematic, not measurements.

<!--
This file is the SOURCE. docs/tutorial-basics.md is generated from it by
`make tutorial-basics`. Write prose normally; every fenced block is executed
by scripts/tutorial.py, and its values are inserted into the generated page.
-->
...
form.to_ipa()
[unit.text for unit in form.units]
len(form.units), len(form.to_ipa())
```
...
matches("t͡ʃ")
matches("[vowel]")
matches("[manner=plosive] / [vowel] _ [vowel]")
```
...
inventory.features["manner"].desc
ipa.wiki("t͡ʃ")
```
...
phrase.to_ipa()
[unit.text for unit in phrase.units]

...
- **`without_boundaries()` refuses an interval even though rebasing now exists.** `rules.rebase` moves a span under the `Edit`s that describe what happened ([rules.md](rules.md#a-tier-survives-the-cascade-because-a-rewrite-rebases-what-it-moved)), and a collapse is not an edit: it lives beside `Edit` in the rule engine, which this module sits below rather than above. So the refusal here is a layering fact, and the way to collapse a form carrying a tier is to state the removal as rules and derive it.

...
- Modifiers contribute by mode: overriding marks replace their base's value (the devoicing ring makes `d̥` voiceless, never both-voiced), additive and secondary marks add only what the base doesn't state, release marks stay phase properties, and prosodic marks live on the unit, outside the feature bag. A mark's mode is read off the features it declares in `ipa.xml` — a mark is a release phase because it says `release=…`, a secondary articulation because it says `velarized=…` — with no per-symbol exception table: pre-glottalization `ˀ` declares `release="glottal"` and the schwa release `ᵊ` declares `release="schwa"`, so neither can fill a slot its base left empty. The mode governs every read of a mark — `scalar()`, `bundle()` and `compose()` share one overlay — so a release-phase mark never speaks for its base: `tˀ` is an alveolar plosive with a glottal phase, and `describe("tˀ")` is not `describe("kˀ")`. A mark's *metadata* never crosses either: `href` and `xsampa` name a symbol, and the symbol a unit's metadata describes is its base, so the article for `tʰ` is the article for `t`, not the one for aspiration.

...
class FormBuilder:
    ...
    def append_ipa(self, text: str, *, strict: bool = False) -> tuple[Any, ...]:
        ...
        for index in range(len(parsed.units)):
            ...
            values["unit-index"] = self._unit_count
...
class _UnitProjection:
    ...
    def intervals(self) -> tuple[Interval, ...]:
        ...
        # Adjacent ticks share their boundary position, so a unit coordinate
        # is the intra-tick gap plus the running gap total of every earlier
        # tick — the same mapping the builder's to_unit computed.
        gap_prefix = [0]
        ...
        if [index for index, _ in indexed] != list(range(len(indexed))):
            raise FormProjectionError("graph interval order is not contiguous")
...
class _FormGraphIndex:
    ...
    @property
    def units(self) -> tuple[Unit, ...]:
        return self._memo("_units", lambda: self.unit_projection.units)

    @property
    def intervals(self) -> tuple[Interval, ...]:
        return self._memo("_intervals", lambda: self.unit_projection.intervals)
...
def _graph_from_units(units: Sequence[Unit], intervals: Sequence[Interval]) -> Any:
    ...
    if extras:
        permitted = frozenset({"interval-index", "unit", "unit-index"})
    ...
    for index, unit in enumerate(units):
        base = {
            "spelling": unit.text,
            "input": True,
            # Keep the caller's frozen Unit as the unit projection.
...
class Form:
    ...
    * Nothing synthesizes one. A form with no dots has no syllable
            ...
            if segment is not None:
                ...
                builder.append_input_atom(
                    ZERO_TIER,
                    {
                        ...
                        "spelling": token,
                        "input": True,
                        "unit": dataclasses.replace(out[-1], timing=None),
                        "unit-index": len(out) - 1,
                    },
                )
    ...
    def to_json(self, *, pretty: bool = False) -> str:
        ...
        Pass pretty=True for indented output using the same native codec.
        The graph includes typed facts, relations, timing, and Form profile
        bindings. Historical units-and-intervals documents are refused.
        """
    ...
    def from_json(cls, data: str | bytes, features: IPAFeatures | None = None) -> Form:
        ...
        The explicit restoring inventory must match the persisted declaration
        identity. This returns a real Form with graph-derived indexes.
        """
        from tiergraph import wire
